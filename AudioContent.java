...
abstract public class AudioContent
{
  ...
  // Play the content via the audio file
	...
	//boolean method, chek if title containts if not audiofile, and return true .contains

	public boolean getWeird(String word)
	{
		if(getTitle().contains(word)) //check if title contains word
		{
			return true;
		}
		else if(getAudioFile().contains(word)) //check if audiofile contains word
		{
			return true;
		}
		else
